...
class TelegramMarkdownRenderer(MarkdownRenderer):
    ...
    def render_heading(self, token: block_token.Heading, max_line_length: int) -> Iterable[str]:
        line = markdown_symbol.get(f'head_level_{token.level}', '')
    ...
    def render_inline_code(self, token: span_token.InlineCode) -> Iterable[Fragment]:
        delimiter = token.delimiter * 3 if len(token.delimiter) == 1 else token.delimiter
        return self.embed_span(Fragment(delimiter + token.padding), token.children, Fragment(token.padding + delimiter))
    ...
    def render_link_reference_definition(self, token: LinkReferenceDefinition) -> Iterable[Fragment]:
        yield Fragment(markdown_symbol.link + formatting.mlink(content=token.title if token.title else token.label, url=token.dest, escape=True))
    ...
    def render_link_or_image(self, token: span_token.SpanToken, target: str) -> Iterable[Fragment]:
        ...
        if token.dest_type == "uri" or token.dest_type == "angle_uri":
            ...
            yield from (Fragment(formatting.escape_markdown("["), title, formatting.escape_markdown(f"][{token.label}]")))
    ...
    def render_auto_link(self, token: span_token.AutoLink) -> Iterable[Fragment]:
        yield Fragment(formatting.escape_markdown(f"<{token.children[0].content}>"))
